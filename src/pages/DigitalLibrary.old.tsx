...
import { BookOpen, GraduationCap, Layers, ArrowLeft, FileText, Download, Loader2 } from "lucide-react";
import {
  Link,
  Outlet,
  useOutletContext,
...
import { ACADEMIC_STRUCTURE } from "@/data/academic-structure";

type IconComponent = ComponentType<SVGProps<SVGSVGElement>>;

interface SemesterResources {
  semesterName: string;
...
const getFileIcon = (mimeType: string): IconComponent => {
  if (mimeType.includes('pdf')) return FileText;
  if (mimeType.includes('image')) return Layers;
  if (mimeType.includes('video')) return GraduationCap;
  return BookOpen;
};
